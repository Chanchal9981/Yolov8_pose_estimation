...
def demo(args):
    ...
    model = torch.load("news/best.pt", map_location='cpu')['model'].float()
    ...
    save = cv2.VideoWriter(f'{path}_.mp4',cv2.VideoWriter_fourcc(*'mp4v'),10,(width,height))
    ...
    while camera.isOpened():
        ...
        if success:
            image = frame.copy()
            # image=cv2.resize(image,(640,640)) ####added
